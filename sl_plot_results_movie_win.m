...
mov =  VideoWriter('linux_level.avi');% avifile('pvc1.avi','quality',qt,'compression','indeo5','fps',fs);
...
%   vy_mtx=reshape(ele(n+1).terms{vy_ele_idx},[inp.nn1-1,inp.nn2-1])';
  ...
  title(sprintf('Simulation %.0f/%.0f  days',bcop(n).tout/3600/24,bcop(end).tout/3600/24))

    
  n_fig=n_fig+1;
  %% sat over x axis 
  subplot('position',[a.left,a.bot-n_fig*a.h_interval-0.025,a.width2,a.height])
  ...
  subplot('position',[a.left,a.bot-n_fig*a.h_interval-0.025,a.width+0.0145,a.height])
 ...
 %     	 ,'w-','linewidth',a.lw);hold on
  ...
  ax4=subplot('position',[a.left,a.bot-n_fig*a.h_interval-0.025,a.width+0.0145,a.height]);
...
%% plot head over time
  ...
  plot(et_x_ay,exchange_flux_mmday,'k-','linewidth',a.lw);hold off
  set(gca,'fontsize',a.fontsize,'FontWeight','bold');
  ax1 = gca;
  ...
  set(get(gca, 'XLabel'), 'Position', vec_pos + [600 4 0]);
  grid on;
  ax1.YRuler.Axle.LineWidth = a.axislinewidth;
  ax1.YRuler.Axle.LineWidth = a.axislinewidth;
